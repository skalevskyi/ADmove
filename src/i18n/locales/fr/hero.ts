/**
 * FR — Hero section.
 */

export const hero = {
  headline_line1: 'Publicité locale à Montpellier',
  headline_line2: 'Votre entreprise est vue chaque jour',
  subheadline:
    'Une visibilité réelle grâce à un véhicule qui circule chaque jour sur le même trajet',
  support: 'Entre Montpellier et le littoral — un trajet réel, vu chaque jour',
  proof: 'Jusqu’à 200 000 vues locales par mois',
  trust: 'Démarrage simple · premier mois à −50 %',
  heroSeoLine1: 'Vous cherchez à rendre votre entreprise plus visible ?',
  heroSeoLine2Before: 'Voir comment fonctionne la ',
  heroSeoLine2Link: 'publicité sur voiture',
  heroSeoLine2After: ' →',
  ctaPrimary: 'Obtenir une estimation',
  ctaSecondary: 'Comment ça fonctionne',
  chip_route: 'Même trajet',
  chip_daily: 'Chaque jour',
  chip_repeat: 'Visibilité répétée',
  imageAlt: 'Véhicule en circulation entre Montpellier et le littoral',
  routeVisualAriaLabel: 'Trajet local quotidien entre Montpellier et le littoral',
  routeVisualVehicleAriaLabel: 'Position actuelle du véhicule sur le trajet',
} as const;
